...
网站里有「证书墙」板块（导航栏"证书"），照片墙风格，目前有 4 张占位证书图。

**方法一（最简单，不用改代码）：**
1. 打开文件夹 `D:\个人网站\public\certificates\`
2. 把你的证书图片命名成和占位图一样的名字，直接覆盖：
   - `cert-1.svg`（证书 1）
   - `cert-2.svg`（证书 2）
   - `cert-3.svg`（证书 3）
   - `cert-4.svg`（证书 4）
...
**方法二（想改证书名称/时间，或加更多证书）：**
打开 `src\data\certificates.ts`，每张证书有：名称（title）、时间（date）、图片路径（image）。
加一张就复制一组 `{ ... }`，并把 image 改成你的文件名，例如 `"/certificates/国家奖学金.png"`。

> 提示：证书图片建议 600×400 左右，显示效果最佳；不修改文件名直接覆盖占位图即可。